...
router.post("/sign-in", validation(signInUserSchema), async (req, res) => {
  try {
    const tokens = await runInTransaction(async (connection) => {
      const authRepository = new AuthRepository(connection);
      const authService = new AuthService(authRepository);
  ...
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const wasEmailChanged = await authService.changeEmail(
  ...
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const wasPasswordChanged = await authService.changePassword(
          (req as MyRequest).userId,
          currentPassword,
          newPassword
        );
...
router.delete("/", auth(), async (req, res) => {
  try {
    await runInTransaction(async (connection) => {
      ...
      const wasUserDeleted = await authService.deleteUser(
        (req as MyRequest).userId
      );
      if (!wasUserDeleted) {
        res.json({ success: false });
