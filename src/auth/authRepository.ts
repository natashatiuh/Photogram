...
export class AuthRepository {
  ...
  async addUserInfo(userId: string, input: SignUpUserInput) {
    const query = `
            INSERT INTO users (id, userName, fullName, dateOfBirth, bio, followers, followings, posts)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `;
    const params = [
      userId,
  ...
  async checkEmailUniqueness(email: string) {
    const query = `
        ...
        `;
    const params = [email];
    const [rows] = await this.connection.execute<IGetUserQueryResult[]>(
      query,
      params
    );
  ...
  async changePassword(
    ...
    const [updateRows] = await this.connection.execute(
      updateQuery,
      updateParams
    );
