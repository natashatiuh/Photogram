...
router.patch(
  "/username",
  validation(changeUserNameSchema),
  auth(),
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const wasUserNameChanged = await usersService.changeUserName(
...
router.patch(
  "/user-full-name",
  validation(changeUserFullNameSchema),
  auth(),
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const wasUserFullNameChanged = await usersService.changeUserFullName(
          (req as MyRequest).userId,
          newUserFullName
        );
...
router.patch(
  "/birth-date",
  validation(changeDateOfBirthSchema),
  auth(),
  async (req, res) => {
    try {
      await runInTransaction(async (connetion) => {
        ...
        const wasBirthDateChanged = await usersService.changeDateOfBirth(
          (req as MyRequest).userId,
          newDate
        );
...
router.patch("/avatar", auth(), upload.single("avatar"), async (req, res) => {
  try {
    await runInTransaction(async (connection) => {
      ...
      const wasAvatarAdded = await usersService.addAvatar(
        (req as MyRequest).userId,
        avatar
      );
      if (!wasAvatarAdded) {
        ...
        res.json({ success: true, file: { filename: avatar } });
      }
    });
  } catch (error) {
    console.log(error);
...
router.patch("/delete-avatar", auth(), async (req, res) => {
  try {
    await runInTransaction(async (connection) => {
      ...
      const wasAvatarDeleted = await usersService.deleteAvatar(
        (req as MyRequest).userId
      );
      if (!wasAvatarDeleted) {
        res.json({ success: false });
...
router.patch("/bio", validation(addUserBioSchema), auth(), async (req, res) => {
  try {
    await runInTransaction(async (connection) => {
      ...
      const { bio } = req.body;
      const wasBioAdded = await usersService.addUserBio(
        (req as MyRequest).userId,
        bio
      );
      if (!wasBioAdded) {
...
router.patch("/delete-bio", auth(), async (req, res) => {
  try {
    await runInTransaction(async (connection) => {
      ...
      const wasBioDeleted = await usersService.deleteUserBio(
        (req as MyRequest).userId
      );
      if (!wasBioDeleted) {
        res.json({ success: false });
  ...
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const wasFollowingOkay = await usersService.followUser(
          (req as MyRequest).userId,
          followedId
        );
        if (!wasFollowingOkay) {
  ...
  async (req, res) => {
    try {
      await runInTransaction(async (connection) => {
        ...
        const { followedId } = req.body;
        const wasUnfollowingOkay = await usersService.unfollowUser(
          (req as MyRequest).userId,
          followedId
        );
...
router.get("/user", auth(), async (req, res) => {
  try {
    const userInfo = await runInTransaction(async (connection) => {
      const usersRepository = new UsersRepository(connection);
      const usersService = new UsersService(usersRepository);
...
router.get("/followers", auth(), async (req, res) => {
  try {
    const followers = await runInTransaction(async (connection) => {
      ...
      const followers = await usersService.getAllUserFollowers(
        (req as MyRequest).userId
      );
      return followers;
    });
...
router.get("/followings", auth(), async (req, res) => {
  try {
    const followings = await runInTransaction(async (connection) => {
      ...
      const followings = await usersService.getAllUserFollowings(
        (req as MyRequest).userId
      );
      return followings;
    });
